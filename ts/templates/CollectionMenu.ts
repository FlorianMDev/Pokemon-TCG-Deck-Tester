...
import { Collection } from "../models/Deck";
...
export class CollectionMenu {
	...
	createHTMLContent() {
		...
		const $loadCollection: HTMLButtonElement = document.createElement('button');
		...
		const $cardCounts: HTMLDivElement = document.createElement('div');
	...
	removeCardFromCount(card: CardData) {
		switch (card.supertype) {
			case 'Pokémon':
				this.collection.pokémonCount--;
				this.$wrapper.querySelector('#pokémon-card-count')!.textContent = `${this.collection.pokémonCount}`;
